// js/tool-handlers.js
// Tool handler factory with dependency injection

export function createToolHandlers({ UIController, ToolsService, processToolCall }) {
    return {
            ...
            try {
                ...
                results = await ToolsService.webSearch(args.query, (result) => {
                    ...
                    UIController.addSearchResult(result, (url) => {
                        if (processToolCall) processToolCall({ tool: 'read_url', arguments: { url, start: 0, length: 1122 } });
                    }, false);
                }, engine);
                if (!results.length) {
