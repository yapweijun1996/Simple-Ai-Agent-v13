/**
 * ./js/utils.js
 * Utilities Module - ES module version
 */
...
export async function fetchWithRetry(url, options = {}, maxAttempts = 3, delay = 1000, timeout = 10000) {
    let attempt = 0;
...
export async function fetchWithProxyRetry(resource, options = {}, proxies = corsProxies, retries = proxies.length, retryDelay = 1000, timeout = 10000) {
    let lastError;
...
export function splitIntoBatches(snippets, maxLen) {
    const batches = [];
    let currentBatch = [];
    let currentLen = 0;
